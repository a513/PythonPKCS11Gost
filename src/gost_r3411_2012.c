...
// ѕредрассчитанные результаты умножени€ 
// всех 256-ти значений байтов на A 
// с преобразованием через Pi[].
...
#endif
// ƒанные сюда поступают всегда порци€ми по 64 байта,
// но count - это размер данных в байтах без учета паддинга.
static void
transform (GOSTR3411_2012_CTX *ctx, const unsigned char *data, unsigned int count)
{
  ...
  // count - в байтах!
  count512[0] = count * 8;
  // «десь нужно выполнить два сложени€
  // в кольце вычетов по модулю 2**512.
  // —начала складываем с count512.
  add512(ctx->N, count512, ctx->N);
#ifdef LISSI_DEBUG
  print_ctx(ctx);
#endif
  // “еперь складываем с исходными данными
...
// «десь важно определитьс€ с пор€дком обработки байтов.
	...
	const unsigned int data_len)
{
  ...
  printf("GOSTR3411_2012_Update\n");
	...
	// ѕри нулевой длине функци€ ничего не измен€ет
	// в контексте.
	if (data_len > 0) {
		unsigned int len = 0;
		if (data == NULL) {
		...
		// ƒанна€ функци€ пытаетс€ накапливать в контексте
		// полные блоки из входных данных и обрабатывать их
		// по мере накоплени€, оставл€€ в контексте остаток
		// в неполном блоке.
		// ≈сли входных данных накопилось меньше 64-х байтов, 
		// то ничего с ними здесь не делаетс€.
		// ≈сли входных данных накопилось больше 64-х байтов, 
		// то из них в цикле вырезаетс€ первый полный блок,
		// который обрабатываетс€ по алгоритму.
		// ѕосле завершени€ цикла остаток сохран€етс€ 
		// в неполном блоке.
		len = ctx->block_len + data_len;
		if (len < 64) {
			// —уществующие данные всегда в начале блока.
			// Ќовые данные копируютс€ после существующих данных
			// побайтно.
			memcpy(ctx->block+ctx->block_len, data, data_len);
			ctx->block_len = len;
		} else {
			// ќставшиес€ существующие данные всегда в начале блока
			// Ќовые данные копируютс€ после существующих данных
			// побайтно.
			unsigned char *p = (unsigned char *)data;
			while (len >= 64)
			{
				// ќтрезаем входные данные с начала 
				// дл€ дополнени€ неполного блока и
				// копируем.
...
//        print_ctx(ctx);
				...
				p += 64-ctx->block_len;
				// «десь обработанный блок всегда полный.
				len -= 64;
				// ќчередной полный блок обработан,
				// поэтому блок контекста пуст.
				memset(ctx->block, 0, 64);
				ctx->block_len = 0;
			}
			if (len > 0) {
				// ќсталс€ неполный блок данных.
				//  опируем его в начало блока контекста побайтно.
...
#endif
	i = ctx->block_len;
	// ѕо крайней мере один байт в блоке свободен,
	// иначе полный блок был бы обработан в Update.
	ctx->block[i++] = 1;
	while (i < 64)
		ctx->block[i++] = 0;
#ifdef LISSI_DEBUG
